...
export default class LodgementService extends StaticService {
    ...
    static deleteApp() {
        ...
        return StorageService.deleteApplication(id).then(() => {
            ...
            return CommonUtil.delay(100).then(() => LodgementService.refreshApplicationList({excludes: [id]}));
        });
    }
    ...
    static refreshApplicationList(options) {
        return StorageService.getApplicationSummaries().then(
            (data) => {
                if (options && options.excludes && data && data.hits) {
                    const filtered = [];
                    data.hits.hits.forEach((item) => {
                        if (options.excludes.indexOf(item['_id']) < 0) {
                            filtered.push(item);
                        }
                    });
                    data.hits.hits = filtered;
                    data.hits.total = filtered.length;
                }
